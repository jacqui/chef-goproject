...
if https_repo_match && https_repo_match[1] && https_repo_match[2]
  destination_directory = File.join(node['goproject']['gopath'], https_repo_match[1], https_repo_match[2])
end

git_repo_match = node['goproject']['repository'].match(/^git@(.*?):(.*?)/)
if git_repo_match && git_repo_match[1] && git_repo_match[2]
  destination_directory = File.join(node['goproject']['gopath'], git_repo_match[1], git_repo_match[2])
end
